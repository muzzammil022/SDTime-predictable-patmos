...
export default function TopNavbar({ onRun, isRunning, onToggleTerminal, terminalVisible }: TopNavbarProps) {
  ...
  return (
    <nav className="h-14 flex items-center px-4 bg-slate-900/85 backdrop-blur-xl border-b border-white/[0.06] flex-shrink-0 z-30 relative">
      {/* Logo */}
      <div className="flex items-center gap-2.5 min-w-[180px]">
        <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-indigo-500/25">
      ...
      <div className="flex-1 flex justify-center px-8">
        <div className="relative w-full max-w-md">
          ...
          <input
            ...
            className={`w-full h-8 pl-9 pr-16 rounded-lg bg-white/[0.04] border text-sm text-slate-200
              placeholder:text-slate-500/60 focus:outline-none search-glow transition-all duration-200
              ${searchFocused ? "border-indigo-500/40 bg-white/[0.07]" : "border-white/[0.06]"}`}
            style={{ fontFamily: "var(--font-sans)" }}
            onFocus={() => setSearchFocused(true)}
            onBlur={() => setSearchFocused(false)}
          />
          <kbd className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-slate-500/70 bg-white/[0.04] px-1.5 py-0.5 rounded border border-white/[0.08]">
            Ctrl+K
          </kbd>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 min-w-[220px] justify-end">
        ...
        <button
          ...
          className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200
            ${terminalVisible ? "text-indigo-400 bg-indigo-500/10" : "text-slate-400 hover:text-slate-200 hover:bg-white/[0.04]"}`}
        ...
        {/* Notifications */}
        <button className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-200 hover:bg-white/[0.04] transition-all duration-200 relative">
          <IconBell size={16} />
          <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-indigo-500 rounded-full ring-2 ring-slate-900" />
        </button>

        {/* Settings */}
        <button className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-200 hover:bg-white/[0.04] transition-all duration-200">
        ...
        <div className="relative ml-1">
          <button
            ...
            className="w-7 h-7 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-[10px] font-bold ring-2 ring-transparent hover:ring-indigo-500/30 transition-all duration-200"
          >
            SD
          </button>
          {avatarOpen && (
            <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800 border border-white/10 rounded-xl shadow-2xl shadow-black/50 py-1.5 z-50">
              ...
              <button className="w-full text-left px-3 py-1.5 text-xs text-slate-300 hover:bg-white/[0.04] transition-colors">
                Profile
              </button>
              <button className="w-full text-left px-3 py-1.5 text-xs text-slate-300 hover:bg-white/[0.04] transition-colors">
                Preferences
